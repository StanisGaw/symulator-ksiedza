...
func _show_next() -> void:
	...
	match item["kind"]:
		...
		"sleep": _show_sleep()
		"bench": _show_bench()
		"phone": _show_phone(item["data"].get("app", "poczta"))
...
func _show_phone(app: String) -> void:
	...
	for id in Phone.APPS:
		...
		if id != "bank":
			...
			if n > 0:
				label += " (%d)" % n
		var button := _button(tabs, label, func() -> void:
			_close_modal()
			_enqueue("phone", {"app": id}))
		button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		button.disabled = id == app
	if app == "bank":
		_phone_bank(box)
	else:
		_phone_inbox(box, app)
	_button(box, "Zamknij", _close_modal)
...
func _phone_bank(box: VBoxContainer) -> void:
	...
	if Game.bank_log.is_empty():
		_text(box, "Konto jeszcze nic nie widziało.", 17)
		return
	var scroll := ScrollContainer.new()
	...
	for entry in Game.bank_log:
		...
		l.text = "dzień %d   %s   %s%s zł" % [int(entry["day"]), str(entry["text"]),
			"+" if amount >= 0 else "-", _money(absi(amount))]
...
func _phone_inbox(box: VBoxContainer, app: String) -> void:
	var indexes: Array[int] = []
	for i in Game.phone_inbox.size():
		if Game.phone_inbox[i].get("app", "") == app:
			indexes.append(i)
	if indexes.is_empty():
		_text(box, "Nic nowego." if app == "media" else "Skrzynka pusta.", 18)
		return
	var scroll := ScrollContainer.new()
	...
	scroll.add_child(list)
	for i in indexes:
		_phone_message(list, i, Game.phone_inbox[i])


func _phone_message(list: VBoxContainer, index: int, msg: Dictionary) -> void:
	var head := Label.new()
