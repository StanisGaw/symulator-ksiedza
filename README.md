...
- Grupy parafialne (2.6): telefon → Parafia pokazuje sześć grup, ich liczebność, zadowolenie, wpływ oraz ostatnią zmianę. Wpływ wynika z liczebności i nastroju; powyżej 70 grupa podejmuje cotygodniową zbiórkę, inicjatywę lub skargę. Dwie silne grupy (wpływ >60) z nastrojem <30 tworzą nazwaną frakcję w radzie. Każda decyzja w wydarzeniu ma jawne, różne skutki dla co najmniej dwóch grup.
- Wspólnoty (2.6): Caritas, świetlica i katecheza mają tygodniowe opłaty 300/450/150 zł w duszpasterstwie. Pierwsza aktywacja opłaca bieżący tydzień, kolejne rozliczenia następują w poniedziałek. Spotkanie na plebanii, raz w tygodniu, zajmuje 60/90/45 minut oraz energię i zwiększa nastrój i liczebność wskazanych grup. Wyłączenie zatrzymuje następne opłaty; ponowne włączenie w tym samym tygodniu nie pobiera opłaty drugi raz. Bank uwzględnia stałe finansowanie nawet gdy spotkanie nie zostało poprowadzone.
- Rozwój księdza (2.5): pięć cech 1–10 rośnie z realnych czynności (start 3, awans cechy co 8XP). Msze i spowiedź budują charyzmę, uczciwe wyjaśnienia wiarygodność, dopięty tydzień zarządzanie, media i festyny wpływy, a krótsze noce bez załamania odporność. Telefon → Rozwój pokazuje profil i doświadczenie.
- Trzy dwupoziomowe drzewka za szacunek (2.5): administrator (tańsze inwestycje/naprawy, przegląd przy biurku), duszpasterz (frekwencja, krótsze odwiedziny), gospodarz (media i uczciwe odpowiedzi kurii). Pierwszy talent kosztuje 20, kolejny 35 i wymaga poprzednika; zakup jest trwały. Opcje wydarzeń mogą wymagać cechy lub skalować pieniądze zarządzaniem — podgląd i wykonanie liczą tę samą kwotę.
...
- Życie religijne (2.4): wynik 0–100 wyliczany rano z ostatnich siedmiu zakończonych dni — 50% frekwencja (cel 1200 uczestnictw), 30% sakramenty (15 spowiedzi), 20% aktywność duszpasterska (cel 5 punktów: odwiedziny po 1, spotkania wspólnot i zakończony festyn po 2). Telefon → Kariera pokazuje składniki, kalendarz, ocenę i trwałą kronikę.
...
**Pliki:**
- `scripts/release_notes.gd` — historia wydań i porównywanie wersji liczbowo (np. 2.10 > 2.9). Przy każdym wydaniu należy zmienić `application/config/version` w `project.godot` i dodać wpis z tą wersją, tytułem i zmianami dla graczy do `RELEASES`. Wpisów historycznych nie usuwamy; `--check` pilnuje obecności bieżącej wersji oraz początkowego 2.3. Treści opisują funkcje dostępne w grze, nie plan przyszłych prac.
- `scripts/ui/release_notes_view.gd` — modal aktualizacji; do podglądu natywnego `--modal=release_notes`.
